...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    const config: any = {
      ...
      SPLAT_FORCE, SHADING, COLOR_UPDATE_SPEED, PAUSED: false,
    ...
    function createDoubleFBO(w: number, h: number, iF: any, f: any, t: any, p: any) {
      ...
      return { width:w, height:h, texelSizeX:fbo1.texelSizeX, texelSizeY:fbo1.texelSizeY, get read(){return fbo1;}, set read(v){fbo1=v;}, get write(){return fbo2;}, set write(v){fbo2=v;}, swap(){const tmp=fbo1;fbo1=fbo2;fbo2=tmp;} };
    }

    function resizeFBO(target: any, w: number, h: number, iF: any, f: any, t: any, p: any) {
      const n = createFBO(w,h,iF,f,t,p); copyProgram.bind(); gl.uniform1i(copyProgram.uniforms.uTexture, target.attach(0)); blit(n); return n;
    }
    ...
    function wrap(v: number, min: number, max: number) { const r=max-min; if(!r) return min; return ((v-min)%r)+min; }
    ...
    function generateColor() {
      ...
      const c=HSVtoRGB(Math.random(),1,1); return {r:c.r*0.15,g:c.g*0.15,b:c.b*0.15};
    }
    ...
    function updatePointerMoveData(p: any, posX: number, posY: number, color: any) {
      p.prevTexcoordX=p.texcoordX; p.prevTexcoordY=p.texcoordY;
      p.texcoordX=posX/canvas.width; p.texcoordY=1-posY/canvas.height;
    ...
    function updatePointerUpData(p: any) { p.down=false; }
    ...
    function step(dt: number) {
      ...
      const vId=velocity.read.attach(0); gl.uniform1i(advectionProgram.uniforms.uVelocity,vId); gl.uniform1i(advectionProgram.uniforms.uSource,vId); gl.uniform1f(advectionProgram.uniforms.dt,dt); gl.uniform1f(advectionProgram.uniforms.dissipation,config.VELOCITY_DISSIPATION); blit(velocity.write); velocity.swap();
    ...
    function drawDisplay(target: any) {
      ...
      displayMaterial.bind(); if(config.SHADING) gl.uniform2f(displayMaterial.uniforms.texelSize,1/w,1/h);
      gl.uniform1i(displayMaterial.uniforms.uTexture,dye.read.attach(0)); blit(target);
    }

    function updateFrame() {
      if (!isActive) return;
      const now=Date.now(), dt=Math.min((now-lastUpdateTime)/1000, 0.016666); lastUpdateTime=now;
      if (scaleByPixelRatio(canvas.clientWidth)!==canvas.width || scaleByPixelRatio(canvas.clientHeight)!==canvas.height) {
        canvas.width=scaleByPixelRatio(canvas.clientWidth); canvas.height=scaleByPixelRatio(canvas.clientHeight); initFramebuffers();
      }
      ...
      step(dt); gl.blendFunc(gl.ONE,gl.ONE_MINUS_SRC_ALPHA); gl.enable(gl.BLEND); drawDisplay(null);
      animationFrameId.current = requestAnimationFrame(updateFrame);
    }

    let firstMove = false;
    function handleMouseDown(e: MouseEvent) { const p=pointers[0]; updatePointerDownData(p,-1,scaleByPixelRatio(e.clientX),scaleByPixelRatio(e.clientY)); clickSplat(p); }
    function handleMouseMove(e: MouseEvent) { const p=pointers[0]; const color=firstMove?p.color:generateColor(); firstMove=true; updatePointerMoveData(p,scaleByPixelRatio(e.clientX),scaleByPixelRatio(e.clientY),color); }
    function handleTouchStart(e: TouchEvent) { const p=pointers[0]; for(const t of Array.from(e.targetTouches)){updatePointerDownData(p,t.identifier,scaleByPixelRatio(t.clientX),scaleByPixelRatio(t.clientY));} }
    function handleTouchMove(e: TouchEvent) { const p=pointers[0]; for(const t of Array.from(e.targetTouches)){updatePointerMoveData(p,scaleByPixelRatio(t.clientX),scaleByPixelRatio(t.clientY),p.color);} }
    function handleTouchEnd(e: TouchEvent) { const p=pointers[0]; for(const _ of Array.from(e.changedTouches)){updatePointerUpData(p);} }
  ...
  return (
    <div style={{ position:'fixed', top:0, left:0, zIndex:5, pointerEvents:'none', width:'100%', height:'100%', mixBlendMode:'screen' }}>
